...
class PostProcess:
    ...
    def __call__(self, preds_batch, org_size_batch, ratios_batch, txy_batch):
        ...
        Args:
            preds_batch (list):     list of predictions in a batch
            org_size_batch (list):  list of (org_img_w, org_img_h) in a batch
            ratios_batch (list):    list of (ratio_x, ratio_y) in a batch when resize-and-center-padding
            txy_batch (list):       list of (tx, ty) in a batch when resize-and-center-padding
        """
...
class pseudo_torch_nms:
    ...
    def non_max_suppression(
        ...
        for xi, x in enumerate(prediction):  # image index, image inference
            ...
            if not n:  # no boxes
                continue
            x_argsort = np.argsort(x[:, 4])[::-1][:max_nms] # sort by confidence and remove excess boxes
            x = x[x_argsort]
            # Batched NMS
